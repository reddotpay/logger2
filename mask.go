...
func MaskCard(s string) string {
	...
	r := regexp.MustCompile(`(?i)<(CardNumber|number|cardnumber|cardnum|cardno|accountnumber)>(\d+)<\/(CardNumber|number|cardnumber|cardnum|cardno|accountnumber)>`)
	...
	r = regexp.MustCompile(`(?i)<(CardCodeValue)>(\d+)<\/(CardCodeValue)>`)
	if m := r.FindStringSubmatch(s); len(m) > 0 {
		s = r.ReplaceAllString(s, fmt.Sprintf("<%s>%s</%s>", m[1], mask(m[2], len(m[2])), m[3]))
	}
	r = regexp.MustCompile(`(?i)<(password)>(\d+)<\/(password)>`)
	if m := r.FindStringSubmatch(s); len(m) > 0 {
		s = r.ReplaceAllString(s, fmt.Sprintf("<%s>%s</%s>", m[1], mask(m[2], len(m[2])), m[3]))
	}
